...
{
    public class SwordBeamWeaponSprite : BasicItem
    {
        private int currentSpeed = 0;
        private bool swordOut = false;
        private readonly int direction;
        private const int swordSpeed = 5, delayUntilSwordSpawn = 30, timePerFrame = 3, itemTimeLimit = 60, preBeamHitboxSizeLong = 11, preBeamHitboxSizeShort = 7;
        ...
        public SwordBeamWeaponSprite(Texture2D itemSpriteSheet, int movingDirection)
        {
        ...
        public override Rectangle ObjectBox(int scale)
        {
            ...
            if (swordOut) returnRect = base.ObjectBox(scale);
            else
            {
                if (direction == 0 || direction == 1)
                    returnRect = new Rectangle((int)pos.X, (int)pos.Y, preBeamHitboxSizeShort * scale, preBeamHitboxSizeLong * scale);
                else
                    returnRect = new Rectangle((int)pos.X, (int)pos.Y, preBeamHitboxSizeLong * scale, preBeamHitboxSizeShort * scale);
            }
            return returnRect;
        }
